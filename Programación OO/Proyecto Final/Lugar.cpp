...
Individuo *Lugar::sacarPersonaje(string identificador)
{
...
bool Lugar::meterPersonaje(Individuo *personajeEnMovimiento)
{
...
bool Lugar::validarMovimiento(vector<Lugar *> posiciones, string *identificador, int cualPosicion)
{
  ...
  if(cualPosicion == 1 || cualPosicion == 3) // Únicos movimientos permitidos.
  {
    if(posiciones[cualPosicion-1])
    {
      if(posiciones[cualPosicion-1]->aforoConEspacio())
      {
        ...
        if(posiciones[cualPosicion-1]->meterPersonaje(personajeEnMovimiento) == false)
          return false;
      }
    }
    else
    {
      ...
      personajeEnMovimiento = posiciones[cualPosicion]->sacarPersonaje(*identificador);
      return false; // Determina que el personaje en cuestión se ahogo.
    }
  }
