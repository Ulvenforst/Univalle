...
void Jugador::jugar()
{
  string validacion_de_entrada = ""; // Descubrí su necesidad al haber puesto "Ñ" como entrada...
  char instruccion = 'Q'; // Intrucción para finalizar el juego.
  ...
  do
  {
    ...
    {
      if(moverPersonaje(instruccion) == 1)
      {
        pintarInterfaz();
    ...
    cin >> (validacion_de_entrada);
    instruccion = validacion_de_entrada[0];
    if(instruccion == 'Q')
      cout << "TE RENDISTE\nPERDISTE" << endl;
...
int Jugador::moverPersonaje(char identificador)
{
